describe('Manage Fields Page', () => {
	...
	it('can sort based on attributes', () => {
		...
		cy.get('tbody tr:first-child td:nth-child(6)').should('contain', '9.19').and('be.visible')
	})
	...
	it('can edit field data', () => {
		...
		cy.wait(500)
		cy.get('tbody tr:first-child td:nth-child(6)').click().type('{selectAll}{backspace}').type('9.10{enter}')
	})
	...
	it('can generate Field Report', () => {
		...
		cy.contains('li', 'Test Field').scrollIntoView().should('be.visible').click()
