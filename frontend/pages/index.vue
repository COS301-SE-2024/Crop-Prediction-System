...
<template>
    <div class="flex flex-col gap-5 h-full">
        <div class="flex space-x-5 w-full">
            <StatPanel v-for="stat in stats" :key="stat.title" :title="stat.title" :chart-data="stat.chartData" :chart-type="stat.chartType" class="w-full" />
        </div>
            ...
            <div class="col-span-2 row-span-2 grid gap-5 h-full">
                ...
                <div class="grid grid-cols-2 gap-5 items-center border border-surface-border p-6 rounded-lg shadow-lg">
                    <div class="flex flex-col gap-2">
                        ...
                        <p>
                            We'd suggest increasing the moisture levels in the soil to improve the overall crop health. This will also help with the current temperature and humidity levels.
                        </p>
                    ...
                    <PolarStat v-for="stat in polarstat" :key="stat.title" :title="stat.title" :chart-data="stat.chartData" :chart-type="stat.chartType" class="w-full" />
                </div>
            </div>
        </div>
    </div>
