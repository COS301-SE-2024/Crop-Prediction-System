<template>
    <div class="w-96 grid gap-5 border border-surface-border p-6 rounded-lg shadow-lg">
        ...
        <div class="flex justify-between">
            <p>
                <span class="text-sm font-[500]" :class="value > 0 ? 'text-green-500' : 'text-red-500'">{{ value > 0 ? '+' + value : value }}%</span>
            ...
            <Tag :severity="convertedValue < 0 ? 'danger' : convertedValue < 0.6 ? 'warning' : 'success'" :value="convertedValue < 0 ? 'Severe' : convertedValue < 0.6 ? 'Moderate' : 'Healthy'" rounded></Tag>
        </div>
    </div>
</template>
...
const value = ref(0);
value.value = Math.floor((props.chartData[props.chartData.length - 1] / props.chartData[props.chartData.length - 2] - 1) * 100);
